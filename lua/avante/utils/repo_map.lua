...
local function iterate_files_in_dir(directory, callback)
  ...
  while true do
    ...
    if type == "directory" and name ~= ".git" and name ~= "node_modules" then
...
function RepoMap.stringify_definitions(filepath)
  -- hassuffix
  if string.sub(filepath, -1) == "~" then return "" end
...
function RepoMap._build_repo_map(project_root)
  local Utils = require("avante.utils")
  local recent_filepaths = Utils.get_recent_filepaths(10)
  local output = {}
  iterate_files_in_dir(project_root, function(filepath)
    local match = false
    for _, fp in ipairs(recent_filepaths) do
      if fp == filepath then
        match = true
        break
      end
...
function RepoMap.get_repo_map()
  if not Config.behaviour.enable_project_context then return nil end
  local Utils = require("avante.utils")
  ...
  local function restore()
    repo_map = RepoMap._build_repo_map(project_root)
  ...
  if not repo_map then
    repo_map = RepoMap._build_repo_map(project_root)
    Path.repo_map.save(project_root, file_ext, repo_map)
  else
    local timer = vim.loop.new_timer()

    if timer then timer:start(0, 30000, vim.schedule_wrap(function() restore() end)) end
  end
  ...
  if handle then
    handle:start(project_root, { recursive = true }, function(err, file)
      if err then
        print("Error watching directory " .. project_root .. ":", err)
        return
      end

      if file and vim.fn.fnamemodify(file, ":e") == file_ext then
        local abs_filepath = PPath:new(project_root):joinpath(file):absolute()
